...
reportSchema.statics.generateSalesReport = async function (startDate, endDate) {
  ...
  const report = await this.create({
    ...
    type: "sales",
    summary: `Total revenue of ₹${totalRevenue.toLocaleString()} from ${totalOrders} orders. ${revenueTrend >= 0 ? "Up" : "Down"} ${Math.abs(revenueTrend).toFixed(1)}% compared to previous period.`,
    period: {
      startDate,
      endDate,
      label: "Custom Period",
